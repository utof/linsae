...
- **The biome precommit gate does NOT auto-format.** lefthook runs `pnpm lint:check` (= `biome check .`, no write), which *fails* on any format drift. So before every `git add`, run `pnpm lint` (= `biome check --write .`) to auto-format, then stage. The commit steps below include this.
...
- **Modify:** `src/shared/types.ts` — append `VideoSource`, `AttachmentKind`, `Attachment`. Additive only (no existing type changed). (`SourceLocator` is intentionally **deferred to Plan 2**, where the note-path first consumes it — adding it here would fail `knip`'s unused-export check.)
- **Modify:** `src/main/db/client.test.ts` — bump the two migration-count assertions (1 → 2) now that `0002` exists. This is the **only** existing file the plan edits; it is a test, not the production note path.

Production code is additive: `src/main/db/migrate.ts`, `client.ts`, and the existing query modules are untouched. The single existing-file edit is the migration-count assertion in `client.test.ts` (Task 1, Step 5).
...
- [ ] **Step 5: Fix the existing migration-count assertions in `client.test.ts`**

Adding `0002` makes the migration count 2, which breaks two pre-existing assertions in `src/main/db/client.test.ts` (the records-migration test and the idempotency test). Update both:

In the "records the migration in _migrations" test (currently asserts length 1 and only `0001`):
```ts
...
In the "is idempotent — running twice…" test:
```ts
...
Expected: all green — the 144-test baseline + the 3 new `0002-schema` tests, with `client.test.ts` now passing.

- [ ] **Step 7: Format, then commit**

```bash
pnpm lint   # biome check --write . — the gate (lint:check) does not auto-format
git add src/main/db/migrations/0002_video_threads.sql tests/integration/0002-schema.test.ts src/main/db/client.test.ts
...
```bash
pnpm lint   # biome check --write . — auto-format before the non-fixing gate runs
...
> **Note:** the `SourceLocator` JSON shape (`{ media, video_id, t? }`) is **not** added here — it has no consumer in Plan 1 and `knip` fails on unused exported types. It is added in Plan 2, where the note-path first reads/writes `notes.source_locator`.
...
- [ ] **Step 6: Run the full suite, format, then commit**

Run: `pnpm exec vitest run` — Expected: all green (the 144-test baseline + all new data-layer tests from Tasks 1–3; ~157 total).
...
- **Spec coverage:** Plan 1 implements spec §Data model's two new tables, the partial indexes, the `ON DELETE SET NULL` orphan behavior, the dedup-by-hash query, and the orphan lifecycle. The `SourceLocator` shape and note-source persistence (`source_kind`/`source_locator` on `notes`, `comment-on` edges) are intentionally deferred to Plan 2 (they edit the existing note path; Plan 1 keeps production code additive, touching only the `client.test.ts` migration-count assertion).
...
Plan 2 (capture/oEmbed/IPC + `app://` shell) consumes the query wrappers and types from Plan 1: `insertAttachment`, `getAttachmentsByHash` (file-dedup), `upsertVideoSource`/`getVideoSource`/`setVideoDuration`, and the `Attachment`/`VideoSource` types. Plan 2 **adds** the `SourceLocator` type (Plan 1 defers it to avoid an unused-export `knip` failure) and extends the existing note path (`save-note.ts`, `frontmatter`, Zod, `replaceLinksForNote` scoping to `edge_type='reference'`, `comment-on` edges) — flagged here so Plan 2's reviewer expects edits to frozen-feeling files.
```
