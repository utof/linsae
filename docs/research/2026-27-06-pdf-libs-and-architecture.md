...
- Mitigations: (1) pin a **current** pdf.js (6.x) and keep it updated; (2) set **`isEvalSupported: false`** in pdf.js options — this is the recommended mitigation in Codean Labs' disclosure and kills the eval-based font path, the exact CVE-2024-4367 vector (which "requires isEvalSupported to be true, which is the default setting," per the Wiz vulnerability database); (3) you already have `sandbox:true`, `contextIsolation:true`, `nodeIntegration:false` — the critical backstop preventing XSS-in-renderer from becoming RCE; (4) keep the strict CSP (no `unsafe-eval` in `script-src`); (5) consider font hardening only if issues arise. Your baseline is already strong; the main action items are **`isEvalSupported:false` + `worker-src blob:` + the MIME entry**.
