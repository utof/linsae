...
- cv engagement: cvSkipped=94 (cv-all, identical all 3 runs — engaged and still catastrophic) / 0 (cull+cv — cv INERT in this mode: the culled set ≈ the visible set, so nothing was ever skipped; that row is effectively "cull plus the per-card cost of carrying `content-visibility:auto`", and it is strictly slower than plain cull). cvSkipped is an end-of-run snapshot at z=1 (engagement oracle), not a peak count.
...
- desynchronized requested=true → actual=**true** per a scripted `getContextAttributes()` probe (Linux x11 + Nvidia — notable; the research expected x11 support to be spotty). PENDING human confirmation while actually drawing.
...
§Recommendations — PASS = meanFps ≥ 55, p95 ≤ 18ms, zero >100ms frames, against measured 60Hz vsync.)
