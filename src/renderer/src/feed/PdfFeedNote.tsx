...
export interface PdfFeedNoteProps {
  ...
  onOpenReader?: () => void
  /**
   * Page-1 thumbnail data-URL generated by usePdfThumbnail (cached by sha256).
   * Null/undefined while the thumbnail is loading or if rendering failed — the
   * card shows only the FileText glyph in the header (no thumbnail strip).
   * When non-null, a fixed-height strip appears at the top of the card.
...
export function PdfFeedNoteContainer({
  ...
  // Page-1 thumbnail: cached by sha256 (staleTime: Infinity) so re-scrolling
  // this card into view in the virtualized feed reuses the cached data-URL
  // rather than re-invoking the pdf.js renderer. @issue utof/linsae#167
  const { data: thumbnailDataUrl } = usePdfThumbnail(meta?.sha256, meta?.mediaUrl)
...
export function PdfFeedNote({
  ...
  return (
    ...
    >
      {/* Page-1 thumbnail strip — additive to the existing header (#167).
          Only rendered when thumbnailDataUrl resolves (cached by sha256 in
          usePdfThumbnail; renders once per session, not per scroll/mount).
          Fixed height keeps the strip layout stable once it appears. While
          loading, the FileText glyph in the header below acts as fallback.
          Clicking opens the PDF reader (same handler as the title button). */}
      ...
      {/* Header: file icon + title + page-count chip + post time.
          FileText glyph acts as the thumbnail fallback while usePdfThumbnail
          is pending or if rendering failed (no strip above = glyph visible). */}
